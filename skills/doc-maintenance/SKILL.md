...
- Phase 完成后的文档审计
- 更新 INDEX.md 路由表
- **任何文档操作完成后** → 顺带检查 MEMORY.md 是否需要精简（规则 9）
...
PK 评审回写目标文档（如 TDD）后，**必须检查涉及的上游文档（如 GDD）是否引用了相同的数值/布局/命名，如不一致则一并修正**。

**触发条件**：任何 PK 评审（tdd-pk-review）的 Phase 2 回写操作
**检查方法**：
1. 遍历每个被接受问题涉及的字段/规格变更（如尺寸、布局、命名、容量）
2. 在上游文档中搜索同一字段的旧值（如 TDD 改了 CD 尺寸 36→48pt，需搜索 GDD 中所有 "36pt" / "36×36" 出现处）
3. 如发现不一致：修正上游文档 + 标注变更来源（如"PK-RN UID-XXX 变更"）+ 更新上游文档版本号和 `last_verified`

**来源**：PK-R4 经验——8 个问题中 5 个因"TDD 回写但 GDD 未同步"产生，均为同类搬运遗漏。

### 规则 9：工作记忆精简（铁律）

每次文档操作完成后，**必须检查 `.workbuddy/memory/MEMORY.md` 是否需要精简**。

**核心原则**：MEMORY.md 的每一行都必须是「违反了会踩坑」的规则或「编码时必须遵守」的决策。过程记录不常驻。

**保留标准（三层分离）**：

| 层级 | 存放位置 | 内容类型 | 行为 |
|------|----------|----------|------|
| **决策宪法** | `MEMORY.md` | 铁律、架构决策、协作约定、技术栈约束 | 长期保留，就地更新 |
| **当日工作台** | `YYYY-MM-DD.md` | 当天做了什么、踩坑、待办 | 7 天后蒸馏有价值内容到 MEMORY.md，其余删除 |
| **归档** | `Docs/Agent/Archive/` 或自然过期 | Sprint 验收记录、PK 评审过程、版本变更明细 | 需要时 read_file 拉回 |

**精简触发条件**：
- MEMORY.md 超过 **60 行** 时必须执行精简
- Sprint/Phase 完成后执行一次精简
- 任何文档维护操作（本 skill 触发时）顺带检查

**精简操作步骤**：
1. 逐行扫描 MEMORY.md，对每一行问：「这行对未来编码/决策有指导作用吗？」
2. **删除**：已完成任务的过程流水（PK 逐轮明细、验收步骤、版本号罗列）
3. **保留**：铁律、架构决策、踩坑经验（会再犯的）、协作约定
4. **压缩**：长描述改为一句话结论（如「14轮PK全部完成」替代逐轮罗列）
5. 更新底部时间戳

**目标**：MEMORY.md 常态 ≤ 55 行，极端情况不超过 70 行。
...
- [ ] 列出所有被接受问题涉及的字段/规格变更
- [ ] 在上游文档（如 GDD）中搜索同一字段的旧值
- [ ] 修正不一致处 + 标注变更来源
- [ ] 更新上游文档版本号和 `last_verified`
...
### 工作记忆精简（规则 9，每次文档操作后检查）

- [ ] 检查 MEMORY.md 行数（`wc -l`），超过 60 行则执行精简
- [ ] 逐行判定：铁律/决策 → 保留 | 过程流水 → 删除或压缩
- [ ] 长描述压缩为一句话结论
- [ ] 确认精简后 ≤ 55 行（极端不超 70 行）
- [ ] 更新底部时间戳
