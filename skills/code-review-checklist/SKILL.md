...
#### CL-9: 架构一致性（🔴 阻塞级 — 2026-05-07 新增）

> 修复或新增代码时，必须确认方案在项目现有架构内解决，未绕过框架另起炉灶。

**验证方法**：

1. **跨场景数据传递**：
   - 是否走 `AppFlowNavigator + IFlowData`？
   - 有没有用 SO 写运行时值 / static / Resources.Load / PlayerPrefs 等 hack 绕过？
   - 直跑场景模式是否与正式流程隔离（不写存档/不改进度）？

2. **修复方案架构合规**：
   - 修复是否在现有框架机制内？（不应该为了修 bug 发明新的通信机制）
   - 如果引入了新模式（如新的单例/事件总线/全局状态），是否有充分理由并经天命人确认？
   - 修复方案是否真机可行？（无 Editor-only API、无 System.IO、无同步阻塞）

3. **Bug 修复路径验证**：
   - 是否先诊断了数据流链路 / 时序关系，找到了真正的断点？
