...
| ADR-017 | RenderBatchManager 桶生命周期 | 初始化按注册表预热，运行时禁止隐式建桶 |
...
| ADR-028 | RuntimeAtlasSystem（v2.0 系统级重构） | 统一渲染管线核心：替代割裂的 6 路渲染架构，按需 Blit，Shelf Packing，Channel 隔离，统一 RBM 提交。Supersedes ADR-007/008/010 运行时约束 |
...
### 关联文档
- 完整技术设计：`docs/Agent/RUNTIME_ATLAS_SYSTEM_TDD.md`（v2.0）
