...
> 💡 详细格式规范、踩坑记录见 `skills/luban-config/` Skill 文档。
...
2. 用 `create_xlsx.py` 创建 xlsx：`python skills/luban-config/scripts/create_xlsx.py -o DataTables/Datas/xxx.xlsx -s TbXxx -f "id:int:ID,..."`
3. 用 `update_tables_extension.py` 自动同步表名：`python skills/luban-config/scripts/update_tables_extension.py --project-root UnityProj`
