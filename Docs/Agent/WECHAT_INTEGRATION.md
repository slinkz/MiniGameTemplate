...
- ✅ `GameStartupFlow` 启动时隐私授权检查（PrivacyDialog → ConfirmDialog 二次确认）
- ⬜ 真实 SDK 实现（按需接入）
...
4. **用户隐私** — 框架内置了隐私授权流程（`CheckPrivacyAuthorize` → `RequirePrivacyAuthorize`），`GameStartupFlow` 会在启动时自动检查。真实实现需对接微信 `wx.requirePrivacyAuthorize` 和 `wx.getPrivacySetting` API
