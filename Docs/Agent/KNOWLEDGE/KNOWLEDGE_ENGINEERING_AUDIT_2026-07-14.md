...
related_docs: Docs/Agent/KNOWLEDGE/KNOWLEDGE_ENGINEERING_ROADMAP.md, Docs/Agent/KNOWLEDGE/KNOWLEDGE_EVALS.md, Docs/Agent/KNOWLEDGE/KNOWLEDGE_INVENTORY.md
---
...
传统 ADR 只记录"为什么这么选"，而本项目的 `ADR/ADR_SCHEMA.md` 将其升级为**可执行的工程约束**：
...
`KNOWLEDGE/KNOWLEDGE_EVALS.md` 定义了 10 个标准评估任务和 6 维度评分规则（路由/效率/一致性/影响面/踩坑/验证）。首批运行得分 8.5/10，发现的具体问题（新增关卡路由、ADR-012 可执行化、模块卡补齐等）已全部修正并复测通过。
