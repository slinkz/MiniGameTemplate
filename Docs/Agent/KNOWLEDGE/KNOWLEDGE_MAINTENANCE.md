...
related_docs: Docs/Agent/AGENT_BOOTSTRAP.md, Docs/Agent/KNOWLEDGE/ARCHITECTURE_REVIEW_PROTOCOL.md, Docs/Agent/templates/DOC_UPDATE_CHECKLIST.md, Docs/Agent/changes/README.md
---
...
| 触发条件 | 必查文档 |
|----------|----------|
| 新增或移动核心代码路径 | `KNOWLEDGE/CODE_KNOWLEDGE_MAP.md`, `INDEX.md` 路由表 B |
| 改变模块职责、边界、入口或数据流 | 对应 `MODULE_CARDS/*.md` |
| 改变常见任务流程或必读文档 | 对应 `CONTEXT_PACKS/*.md`, `AGENT_BOOTSTRAP.md` |
| 改变架构决策或实现状态 | ADR 原文、`ADR/ADR_SCHEMA.md`, `ADR/ADR_INDEX.md` |
...
| 改变玩法规则、关卡节奏、敌人/技能/Buff/道具设计 | `ROLES/DESIGNER_BOOTSTRAP.md`, `DESIGN/README.md`, 对应 `DESIGN/*.md` |
| 改变 UI token、组件规格、界面状态、动效或文案 | `ROLES/UI_AGENT_BOOTSTRAP.md`, `UI_DESIGN/README.md`, 对应 `UI_DESIGN/*.md` |
| 新增或替换 sprite、VFX、UI icon、audio、font、background 等关键资产 | `ROLES/ART_ASSET_AGENT_BOOTSTRAP.md`, `ASSET_PIPELINE/README.md`, `ASSET_PIPELINE/ASSET_MANIFEST.md` |
| 改变渲染、RuntimeAtlas、VFX、飘字排查方式 | `DEBUG_PLAYBOOK.md`, Rendering/Danmaku 模块卡 |
| 改变微信、云存储、CDN、广告、构建流程 | `PLATFORM/WECHAT_INTEGRATION.md`, `Docs/Guide/BUILD_MINIGAME.md` |
...
6. 如果新增核心路径，更新 `KNOWLEDGE/CODE_KNOWLEDGE_MAP.md`。
7. 如果改变架构约束，更新 ADR 原文与 `ADR/ADR_SCHEMA.md`。
...
| `KNOWLEDGE/CODE_KNOWLEDGE_MAP.md` | 代码路径到文档/ADR/验证项映射 | 新增核心路径或验证项变化 |
...
4. 是否需要更新 `KNOWLEDGE/KNOWLEDGE_INVENTORY.md` 的 Skill 盘点。
5. 是否需要更新 `KNOWLEDGE/CODE_KNOWLEDGE_MAP.md` 的 Skill 映射。
...
检查范围：
- `KNOWLEDGE/CODE_KNOWLEDGE_MAP.md`：代码路径 → Module Card → Context Pack → TDD/Workflow 引用的存在性
- `ADR/ADR_SCHEMA.md`：AppliesTo 路径的存在性
- `INDEX.md`：文档引用的存在性
