...
> 文档版本：v2.0-draft
> 创建日期：2026-04-18
> 修订日期：2026-04-18（v2.0 — 系统级重构定位）
> 作者：广智 × 天命人
> 状态：**设计评审中** — 待天命人确认后转入开发
...
> **ADR-029 v2 后，RenderLayer 维度已彻底移除。BucketKey 降维为纯 Texture。**
>
> ```
...
（与 v1.0 相同，不再赘述。详见 v1.0 §3.3。）

### 3.4 Blit 策略（WebGL 兼容）

（与 v1.0 相同。详见 v1.0 §3.4。）
...
v2.0 统一渲染管线初始化：
  ...
  2. 各 Channel 预热：
     - Bullet: 遍历 BulletTypeSO → Allocate → 收集 (Layer, AtlasRT) BucketKey
     - Laser:  遍历 LaserTypeSO → Allocate → 收集 (Layer, AtlasRT) BucketKey
     - VFX:    遍历 VFXTypeSO → Allocate → 收集 (Layer, AtlasRT) BucketKey
     - DmgText: 将 NumberAtlas Allocate → 收集 BucketKey
     - Trail:  特殊处理（纯顶点色 → whiteTexture 桶）
  3. 统一 RBM 实例（或少量 RBM 实例）初始化
  4. 各 Renderer 拿到统一 RBM 引用
```
...
| 2 | LaserRenderer | 已用 RBM | 改用 RuntimeAtlas 纹理 | 🟢 低 | LaserTexture → RuntimeAtlas |
| 3 | LaserWarningRenderer | 已用 RBM | 改用 RuntimeAtlas 纹理 | 🟢 低 | 同 LaserRenderer |
...
### 5.3 LaserRenderer / LaserWarningRenderer 迁移（难度：🟢 低）
...
**迁移后**：
```csharp
// Initialize: 遍历 LaserTypeSO → runtimeAtlas.Allocate(Laser, lt.LaserTexture)
//             → 收集 (Layer, AtlasRT) BucketKey
// Rebuild:    runtimeAtlas.GetAllocation(lt.LaserTexture) → TryGetBucket → WriteSegmentQuad
```

**注意**：激光使用的 UV 是 `(0, uvYAccum) → (1, uvYEnd)` 的纵向映射，不是 SpriteSheet 切分。迁移时需要确保 AtlasAllocation.UVRect 内的 UV 映射正确。

> **⚠️ 未决项 UD-10**：激光纹理的 UV 是全贴图映射（0→1），Blit 到 Atlas 子区域后，LaserRenderer 的 UV 计算需要从 `(0,0)→(1,1)` 变为 `(uvRect.x, uvRect.y)→(uvRect.xMax, uvRect.yMax)` 的变换。这需要修改 `WriteSegmentQuad` 的 UV 计算逻辑。
...
private void RunLateUpdatePipeline()
{
    ...
    // 统一提交
    _renderBatchManager.UploadAndDrawAll(); // 一次提交全部
...
>
> **风险**：单 RBM 的 BucketKey 空间变大（Bullet + Laser + VFX + DmgText 的所有 (Layer, Texture) 组合），但实际桶数仍然可控（每 Channel 1-4 张 Atlas × 2 Layer = 10-20 桶）。
...
| BC-08 | RT Lost（WebGL Tab 切换） | 标记 dirty，重新 Blit 所有缓存条目 |
...
| **BC-11** | **激光 UV 从全贴图变为子区域** | LaserRenderer 的 UV 计算需要适配 Atlas 子区域映射 |
...
| **LaserRenderer UV 映射错误** | 中 | 高 | 激光 UV 映射需要独立测试 |
| **迁移期间两套系统并存的混乱** | 低 | 中 | 分 Phase 迁移，每 Phase 保持可运行 |

### 6.3 未决项清单（UD - Undecided）

| ID | 问题 | 待定方向 | 决策影响 |
|----|------|----------|----------|
| **UD-01** | BucketKey.Texture 从 `Texture2D` 拓宽为 `Texture` 的影响面 | 方案 A（推荐）vs 方案 B（Material 注入） | RBM + 所有 Renderer |
| **UD-02** | Atlas 溢出时运行时创建新桶？ | 选项 1：预分配 MaxPages 桶（保守）<br>选项 2：受控建桶（灵活） | ADR-015 + RBM |
| **UD-03** | WebGL RT Lost 重建策略 | 选项 1：全量重 Blit<br>选项 2：懒重建 | 缓存数据结构 |
| **UD-04** | 源纹理 Blit 后是否可卸载？ | 保持引用（支持重建）vs 释放 | 内存策略 |
| **UD-05** | 是否支持热更新纹理？ | 第一版不支持，预留接口 | API 设计 |
| **UD-06** | 是否引入 `RuntimeAtlasConfigSO`？ | 推荐是 | 配置方式 |
| **UD-07** | 溢出时回退到独立贴图还是不渲染？ | 推荐回退 | Renderer 逻辑 |
| **UD-08** | ~~ADR-007/008/010 是否标记 Superseded？~~ | ✅ **已确认**：标记 Superseded by ADR-028（运行时），Editor 环境仍生效 | ADR 体系 |
| **UD-09** | 运行时是否完全忽略 AtlasBinding？ | 推荐是（统一管线无两条路径） | 纹理解析链 |
| **UD-10** | 激光 UV 从全贴图到 Atlas 子区域的映射方案？ | 需修改 WriteSegmentQuad UV 计算 | LaserRenderer |
| **UD-11** | TrailPool 迁移方案 A（独立+统计）vs B（Quad化）？ | 推荐 A | TrailPool |
| **UD-12** | 全局单 RBM vs 多 RBM？ | 推荐单 RBM | 渲染调度 |
...
（与 v1.0 相同，API 签名不变。详见 v1.0 §6.1。）
...
| R0.2 | 数据结构：`AtlasChannel`, `AtlasAllocation`, `AtlasChannelConfig`, `Shelf` | `.cs` 文件 |
| R0.3 | `ShelfPacker` 算法实现 + 单元测试 | `.cs` + test |
| R0.4 | `AtlasBlit` 适配层 + `Hidden/RuntimeAtlasBlit` Shader | `.cs` + `.shader` |
| R0.5 | `AtlasPage` 生命周期管理 | `.cs` |
...
| Task | 描述 | 交付物 |
|------|------|--------|
| R2.1 | `BucketKey.Texture` 类型拓宽（`Texture2D → Texture`） | `RenderBatchManager.cs` |
| R2.2 | BulletRenderer 集成 RuntimeAtlas | `BulletRenderer.cs` |
| R2.3 | LaserRenderer 集成 RuntimeAtlas（含 UV 映射适配） | `LaserRenderer.cs` |
...
| Task | 描述 | 交付物 |
|------|------|--------|
| R3.1 | DamageNumberSystem 迁移到 RBM（含 UV 切分适配） | `DamageNumberSystem.cs` |
| R3.2 | TrailPool 方案落地（方案 A：接入统计 / 方案 B：Quad 化） | `TrailPool.cs` |
| R3.3 | 统一渲染调度：DanmakuSystem.RunLateUpdatePipeline 改造 | `DanmakuSystem.UpdatePipeline.cs` |
| R3.4 | VFX 系统 SpriteSheetVFXSystem 接入统一管线 | `SpriteSheetVFXSystem.cs` |
...
| 风险 | 可能加的天数 | 触发条件 |
|------|------------|---------|
| 激光 UV 映射复杂度超预期 | +1 天 | 激光 UV 滚动 + Atlas 子区域的交叉问题 |
...
| # | 验收项 | 通过标准 |
|---|--------|----------|
| AC-01 | 子弹使用 RuntimeAtlas 合批 | 10 种不同贴图的子弹，DrawCall ≤ 2（Normal + Additive） |
| AC-02 | VFX 使用 RuntimeAtlas 合批 | 5 种不同特效，DrawCall ≤ 2 |
...
| **AC-07** | **激光使用 RuntimeAtlas** | 激光纹理通过 Atlas 合批，UV 滚动正确 |
...
| PC-04 | 全局 DrawCall | N×2+ DC | **≤ 10 DC**（所有 Channel × Layer + Trail） |
