...
> 📐 完整的 ASCII 架构图参见 [Agent/ARCHITECTURE.md — 架构总览](../Agent/ARCHITECTURE.md#架构总览)，下面是面向人类开发者的简化说明。

```
 游戏逻辑层（_Game/ 业务代码 + _Example/ 示例代码）
       ↕ 引用 SO 资产 / 监听 SO 事件
 ScriptableObject 资产层（Variables, Events, Config, Audio, FSM）
       ↕ 驱动
 框架服务层（UIManager, AudioManager, TimerService, PoolManager …）
```

> **当前示例组织（2026-04-11）**
> - `Assets/_Example/ClickGame/`：点击计数器示例，包含 `Scenes/ClickGame.unity`、`Scripts/`、`UI/ClickGame/`
> - 主菜单提供 `示例 Demo` 分组，包含 ClickGame 入口；示例内通过返回按钮或 `Esc` 返回主菜单（重载 `Boot` 场景）
...
> 📐 完整的启动流程图（含 IStartupFlow 接口、配置表 Lazy Deserialization 等）参见 [Agent/ARCHITECTURE.md — 启动流程](../Agent/ARCHITECTURE.md#3-启动流程)。
...
> 📐 精确的层级 × 模块依赖表参见 [Agent/ARCHITECTURE.md — 模块依赖关系](../Agent/ARCHITECTURE.md#模块依赖关系)。
...
**实际意义**：
- `AudioSystem`(L2) 可以引用 `DataSystem`(L1)
- `UISystem`(L2) **不能**引用 `FSM`(L3)
- `_Game/` 代码可以引用任何框架模块
...
> 📐 Agent 视角下的简洁版设计决策参见 [Agent/ARCHITECTURE.md — 关键设计决策](../Agent/ARCHITECTURE.md#关键设计决策)。
