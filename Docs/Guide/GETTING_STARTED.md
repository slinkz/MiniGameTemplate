...
### 为什么需要 code-review-checklist？

AI Agent 生成的代码经常会犯「人类不太会犯但 AI 反复犯」的错误：改了 C# 但忘了改 FairyGUI 源文件、移动代码时忘了同步移动生成代码、跨命名空间引用时写错 namespace……这些错误模式如果不系统化记录，每次换会话都可能重蹈覆辙。

`code-review-checklist` 将所有历史踩坑经验提炼为 **8 类结构化检查清单**（CL-1 ~ CL-8），代码改动后按清单逐项验证，把「事后救火」变成「事前拦截」。

**如何避免规则膨胀？**

随着项目推进，踩坑记录会不断增多。如果不加控制，经验库本身就会变成负担——上下文溢出、审查变慢、无关条目干扰判断。为此，Skill 内建了**分层加载 + 定期蒸馏**机制：

- **活跃层**（`known-pitfalls.md`）：只保留最近 30 条 + 标记为 `[经典]` 的高频条目，每次审查必读
- **归档层**（`known-pitfalls-archive.md`）：已被检查清单覆盖、重复模式合并后的旧条目，仅在活跃层未匹配时按需查阅
- **蒸馏触发**：活跃层超过 30 条时自动执行蒸馏——已固化到 CL 的条目降级、重复模式合并、90 天未命中的条目归档
- **经典条目免蒸馏**：标记 `[经典]` 的条目是高频复发错误，永远留在活跃层

这套机制确保经验库始终保持精简高效，不会随项目寿命无限膨胀。
