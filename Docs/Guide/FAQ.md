...
### Q: 控制台报 "[UIManager] Failed to create panel XXX"

**原因**：FairyGUI Extension 未正确注册，或包名/URL 不匹配。

**排查步骤**：
1. 检查 `GameStartupFlow.RunAsync` 中是否调用了 `UIManager.RegisterBinder("PackageName", PackageName.XXXBinder.BindAll)`
2. 确认 FairyGUI 编辑器中 `package.xml` 包含 `genCode="true"`
3. 确认导出的 C# 代码（`XXXPanel.cs`）中 `URL` 常量和 `ConstructFromXML` 正确
...
**原因**：`AssetService` 尚未初始化就尝试打开 UI。所有 UI 加载通过 YooAsset 异步完成，`AssetService` 必须先初始化。

**解决**：确保 `GameBootstrapper` 中 `AssetService` 初始化完成后再打开 UI：
```csharp
// ✅ 正确 — 通过 async 打开面板
...
**原因**：对话框的 `PanelSortOrder` 低于当前最上层面板，被遮挡了。最常见场景：在 LoadingPanel（SortOrder = 600）显示期间弹出 Dialog（默认 SortOrder = 300），对话框被 LoadingPanel 完全覆盖。

**解决**：在 `XXXPanel.Logic.cs` 中 `PanelSortOrder` 返回更高的值：
```csharp
...
> 📝 v0.5.0 已修复此问题：PrivacyDialog 和 ConfirmDialog 在启动流程中使用的 PanelSortOrder 已设为 700。

### Q: 对话框被拉伸为全屏

**原因**：`IUIPanel.IsFullScreen` 返回了 `true`。全屏面板会调用 `MakeFullScreen()` 拉伸面板。

**解决**：对话框的 `IsFullScreen` 应返回 `false`，并实现 `IModalDialog` 接口以获得自动居中和半透明遮罩。
...
> 💡 项目中提供了 `luban-config` Skill（仓库源目录 `skills/luban-config/`；运行时目录按工具可能为 `.workbuddy/skills/luban-config/` 或 `.codebuddy/skills/luban-config/`），AI 助手会自动使用它完成配置表操作。
